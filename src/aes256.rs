//! Pure AES-256 block cipher (encrypt + decrypt).
...
pub struct Aes256 {
  ek: [u8; 240], // expanded key: 15 round keys * 16 bytes
}

impl Aes256 {
  pub fn new(key: &[u8; 32]) -> Self {
    let mut ek = [0u8; 240];
    ek[..32].copy_from_slice(key);
    for i in 8..60 {
      let mut t = [ek[4 * (i - 1)], ek[4 * (i - 1) + 1], ek[4 * (i - 1) + 2], ek[4 * (i - 1) + 3]];
      if i % 8 == 0 {
        t.rotate_left(1);
        for b in t.iter_mut() {
          *b = SBOX[*b as usize];
  ...
  pub fn encrypt_block(&self, block: &mut [u8; 16]) {
    add_round_key(block, &self.ek, 0);
    for round in 1..14 {
      sub_bytes(block);
      shift_rows(block);
      mix_columns(block);
      add_round_key(block, &self.ek, round);
    }
    sub_bytes(block);
    shift_rows(block);
    add_round_key(block, &self.ek, 14);
  }
...
fn add_round_key(state: &mut [u8; 16], ek: &[u8; 240], round: usize) {
  let off = round * 16;
  for i in 0..16 {
    state[i] ^= ek[off + i];
  }
