#!/bin/bash
# Decrypt a file (AES-256-CBC, PBKDF2, base64, Rust implementation).
# Usage: rdec.sh <input_file> <output_file> [password]
# If password is omitted, it is read from the terminal.
# Compatible with dec.sh; decrypts output from enc.sh or renc.sh.
...
if [ -n "$PASS" ]; then
  "$BIN" dec "$INPUT" "$OUTPUT" "$PASS"
else
  "$BIN" dec "$INPUT" "$OUTPUT"
fi
